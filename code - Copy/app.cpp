...
internal bool32
CompileShader(memory_arena *Arena,
...
internal void
InitProgram(memory_arena *Arena,
            ...
            char *FragmentShaderFileName, char **Attributes, u32 NumAttrib)
{
    ...
    // Compile Shaders
    CompileShader(Arena, VertexFileResult.Memory,
                  VertexFileResult.Size, "Vertex Shader", VertexShaderID);
    CompileShader(Arena, FragmentFileResult.Memory,
    ...
    for(u32 CurrentAttribute = 0;
        ...
        CurrentAttribute++)
    {
        glBindAttribLocation(Program->ID, Program->NumAttrib,
    ...
    if (isLinked == GL_FALSE)
    {
        ...
        glGetProgramInfoLog(Program->ID, maxLength, &maxLength, errorLog);
    ...
    glDetachShader(Program->ID, VertexShaderID);
    glDetachShader(Program->ID, FragmentShaderID);
    glDeleteShader(VertexShaderID);
    glDeleteShader(FragmentShaderID);
}
              ...
              thread_context *Thread, app_memory *Memory)
{
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(render_vertex),
                          (void*)OffsetOf(render_vertex, X));
    //This is the color attribute pointer
    glVertexAttribPointer(1, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(render_vertex),
                          (void*)OffsetOf(render_vertex, R));
    //This is the UV attribute pointer
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(render_vertex),
    ...
    InitProgram(TransientArena, Thread, Memory, TextureProgram,
                "texture_shading.vert", "texture_shading.frag",
                TextureProgramAttributes,
                ArrayCount(TextureProgramAttributes));
    InitProgram(TransientArena, Thread, Memory, LineProgram,
    ...
    GLint textureUniform = glGetUniformLocation(TextureProgram, "textureSampler");
                       ...
                       u32 *MaxChunkX, u32 *MaxChunkY)
{
    ...
    v2 BiggestEntityHalfDims = {64.f, 64.f};
    v2 Min = MinPos - HalfDims - BiggestEntityHalfDims;
    Min.X = Maximum(0.f, Min.X);
    Min.Y = Maximum(0.f, Min.Y);
    v2 Max = MaxPos + HalfDims + BiggestEntityHalfDims;
...
GetChunksFromBox(world *World,
                       v3 MinPos, v3 MaxPos, v3 HalfDims,
                       ...
                       u32 *MaxChunkX, u32 *MaxChunkY, u32 *MaxChunkZ)
{
    ...
    v3 BiggestEntityHalfDims = {64.f, 64.f, 64.f};
    v3 Min = MinPos - HalfDims - BiggestEntityHalfDims;
    Min.X = Maximum(0.f, Min.X);
    Min.Y = Maximum(0.f, Min.Y);
    Min.Z = Maximum(0.f, Min.Z);
    v3 Max = MaxPos + HalfDims + BiggestEntityHalfDims;
    Max.X = Minimum(TileMapWidth, Max.X);
    Max.Y = Minimum(TileMapHeight, Max.Y);
    Max.Z = Minimum(TileMapDepth, Max.Z);

    *MinChunkX = (u32)(Min.X / ChunkWidth);
    *MinChunkY = (u32)(Min.Y / ChunkHeight);
    *MinChunkZ = (u32)(Min.Z / ChunkDepth);
    
    *MaxChunkX = (u32)(Max.X / ChunkWidth);
    *MaxChunkY = (u32)(Max.Y / ChunkHeight);
    *MaxChunkZ = (u32)(Max.Z / ChunkDepth);
}
...
internal bool32
TestWall(float WallX, float RelX, float RelY, float PlayerDeltaX, float PlayerDeltaY,
...
//    DDEntityZ -= (10.f * Entity->VelocityZ);
        ...
        Iteration++)
    {
        ...
        v3 EntityHalfDims = Entity->CollisionHalfDims;
        ...
        v3 MinkowskiDiameter = Entity->CollisionHalfDims;
        
        u32 EntityTileWidth = CeilFloatToUInt32(MinkowskiDiameter.X /
                                                CollisionTileWidth);
        u32 EntityTileHeight = CeilFloatToUInt32(MinkowskiDiameter.Y /
                                                 CollisionTileHeight);    
        u32 EntityTileDepth = CeilFloatToUInt32(MinkowskiDiameter.Z /
                                                 CollisionTileDepth);
                ...
                ChunkX++)
            {
                    ...
                    ChunkZ++)
                {
                    world_chunk *ThisChunk = GetChunk(World, ChunkX,
                                                      ChunkY, ChunkZ);
                    world_entity_chunk *EntityChunk = &ThisChunk->FirstEntityChunk;
                    while(EntityChunk)
                    {
                            ...
                            EntityIndex++)
                        {
                            ...
                            {
                                v2 MinCorner;
                                MinCorner.X = -MinkowskiDiameter.X -
                                    ThisEntity->CollisionHalfDims.X;
                                MinCorner.Y = -MinkowskiDiameter.Y -
                                    ThisEntity->CollisionHalfDims.Y;
                                v2 MaxCorner = {ThisEntity->CollisionHalfDims.X,
                                                ThisEntity->CollisionHalfDims.Y};
                                MaxCorner.X += MinkowskiDiameter.X;
                                MaxCorner.Y += MinkowskiDiameter.Y;

                                v3 Rel = EntityOldPosition -
                                    ThisEntity->Position;
                
                                if (TestWall(MinCorner.X, Rel.X, Rel.Y, EntityDelta.X, EntityDelta.Y,
                                ...
                                if (TestWall(MaxCorner.X, Rel.X, Rel.Y, EntityDelta.X, EntityDelta.Y,
                                             &tMin, MinCorner.Y, MaxCorner.Y))
                                {
                                    WallNormal = {1, 0};
                                    Collided = true;
                                ...
                                if (TestWall(MinCorner.Y, Rel.Y, Rel.X, EntityDelta.Y, EntityDelta.X,
                                ...
                                if (TestWall(MaxCorner.Y, Rel.Y, Rel.X, EntityDelta.Y, EntityDelta.X,
                                             &tMin, MinCorner.X, MaxCorner.X))
                                {
                                    WallNormal = {0, -1};
                                    Collided = true;
            ...
            EntityIndex++)
        {
            ...
            world_entity *ThisEntity = &World->Entities[EntityIndex];
            if (ThisEntity != Entity)
            {
                v2 MinCorner;
                MinCorner.X = -EntityHalfDiameterW -
                    ThisEntity->CollisionHalfDims.X;
                MinCorner.Y = -EntityHalfDiameterH -
                    ThisEntity->CollisionHalfDims.Y;
                v2 MaxCorner = {ThisEntity->CollisionHalfDims.X,
                                ThisEntity->CollisionHalfDims.Y};
                MaxCorner.X += EntityHalfDiameterW;
                MaxCorner.Y += EntityHalfDiameterH;

                v2 Rel = EntityOldPosition -
                    ThisEntity->Position;
            ...
            TileY++)
        {
            for(u32 TileX = MinTileX;
                ...
                TileX++)
            {
                bool32 IsCollidable = GetCollisionValue(World,
                                                        TileX,
                                                        TileY);
                v2 TileP;
                TileP.X = (float)TileX * CollisionTileWidth;
                TileP.Y = (float)TileY * CollisionTileHeight;
            
                if (IsCollidable)
                {
                    v2 MinCorner = {-EntityHalfDiameterW, -EntityHalfDiameterH};
                    v2 MaxCorner = {(float)CollisionTileWidth, (float)CollisionTileHeight};
                    MaxCorner.X += EntityHalfDiameterW;
                    MaxCorner.Y += EntityHalfDiameterH;

                    v2 Rel = EntityOldPosition.Absolute - TileP;
        ...
        if (CollidedEntity && !CanCollide(AppState, Entity->Type,
                                       CollidedEntity->Type))
        {
            tMin = 1.f;
            WallNormal = {};
        ...
        Entity->Position.Z = Maximum(0.f, Entity->Position.Z);
          ...
          world *World, memory_arena *Arena, v3 Position)
{
    ...
    Player->Texture = &Assets->ZoubirTexture;
    Player->ShadowTexture = &Assets->ShadowTexture;
    Player->TextureNumTilesX = 4;
    Player->TextureNumTilesY = 4;
    Player->TextureOrigin = {16, 42};
    Player->CollisionHalfDims = {10, 4};
          ...
          world *World, memory_arena *Arena, v3 Position)
{
    ...
    Entity->Texture = &Assets->ZoubirTexture;
    Entity->ShadowTexture = &Assets->ShadowTexture;
    Entity->TextureNumTilesX = 4;
    Entity->TextureNumTilesY = 4;
    Entity->TextureOrigin = {16, 42};
    Entity->CollisionHalfDims = {10, 4};
            ...
            world_entity *Player)
{
    ...
    Entity->AnimationState = {};
    Entity->Texture = &Assets->FamiliarTexture;
    Entity->ShadowTexture = &Assets->ShadowTexture;
    Entity->TextureNumTilesX = 2;
    Entity->TextureNumTilesY = 1;
    Entity->TextureOrigin = {22 * 0.5f, 12 * 0.5f};
    Entity->CollisionHalfDims = {22 * 0.5f, 12 * 0.5f};
            ...
            v3 Velocity)
{
    ...
    Entity->Position = CastPosition;
    Entity->Velocity = Velocity;
    Entity->Dimensions = {36 * 0.5f, 36 * 0.5f};
    Entity->AnimationState = {};
    Entity->Texture = &Assets->FireBallTexture;
    Entity->ShadowTexture = &Assets->ShadowTexture;
    Entity->TextureNumTilesX = 4;
    Entity->TextureNumTilesY = 4;
    Entity->TextureOrigin = {18 * 0.5f, 18 * 0.5f};
    Entity->CollisionHalfDims = {18 * 0.5f, 18 * 0.5f};
    ...
    *AnimationSpeed = 1.f;
    ...
    if (Input->ArrowRight.Pressed)
    {
        ...
        world_entity *Entity = AddFireBall(AppState, World, Arena,
                                           Player, Player->Position + v3{32.f, 0.f, 30.f},
    ...
    if (Input->ArrowDown.Pressed)
    {
        ...
        world_entity *Entity = AddFireBall(AppState, World, Arena,
                                           Player, Player->Position + v3{0.f, 32.f, 30.f},
...
{
    v3 EntityCameraPosition = Entity->Position - CameraOffset;
    v2 EntityTexturePosition = EntityCameraPosition.XY - Entity->TextureOrigin;
    EntityTexturePosition.Y -= Entity->Position.Z;
    v3 EntityCollisionRectPosition = EntityCameraPosition - Entity->CollisionHalfDims;

    v3 ShadowOrigin = {7, 4};
    v3 ShadowDims = {14, 7};
    v3 ShadowPosition = EntityCameraPosition - ShadowOrigin;                                                             
                                                           
    float SortingValue =
        Entity->Position.Y;
    BeginBatch(RenderContext, Entity->Texture->ID,
    ...
    RenderQuadTexture(RenderContext,
                      ...
                      Entity->Dimensions.Y,
                      Entity->Uvs,
                      RGBA8_WHITE, Entity->Position.Z);
    EndBatch(RenderContext);
    if (Entity->ShadowTexture)
    {
    BeginBatch(RenderContext, Entity->ShadowTexture->ID,
               Entity->Position.Y, TextureProgram);

    ColorRGBA8 ShadowColor;
    ShadowColor.ColorU32 = RGBA8_WHITE;
    if (Entity->Position.Z < 255 / 3)
    {
        ShadowColor.A = (u8)(255 - Entity->Position.Z * 3);
    }
    else
    {
        ShadowColor.A = 0;
    ...
    }
    {
        ...
        DrawRectangle(RenderContext,
                      EntityCollisionRectPosition.X,
                      EntityCollisionRectPosition.Y,
                      Entity->CollisionHalfDims.X * 2,
                      Entity->CollisionHalfDims.Y * 2,
...
extern "C" APP_UPDATE_AND_RENDER(AppUpdateAndRender)
{
    ...
    {        
        ...
        Assets->TileMap1 =
            LoadTexture(Thread, Memory, "tile_map_1.zi", TEXTURE_NO_FILTER);
        ...
        // to a struct
        Assets->ZoubirAudio = LoadWav(Thread, Memory, "Soda.wav");
        Assets->Dash = LoadWav(Thread, Memory, "Dash.wav");
        Assets->FireCast = LoadWav(Thread, Memory, "fire_cast.wav");
        

        world *World = &AppState->World;
        World->TileWidth = 32;
        World->TileHeight = 32;
        World->TileDepth = 32 * 32;
        
        World->CollisionWidth = 32;
        World->CollisionHeight = 32;
        World->CollisionDepth = 32 * 32;
        ...
        u32 const CollisionToTilesX =
            World->TileWidth / World->CollisionWidth;
        u32 const CollisionToTilesY =
            World->TileHeight / World->CollisionHeight;
        ...
        gl_texture TMT = Assets->TileMap1;
...
//                Tiles[CurrentTile].Index = 4;
    ...
    gl_texture *DragonTexture = &Assets->DragonTexture;
    gl_texture *ZoubirTexture = &Assets->ZoubirTexture;
...
//  gl_texture ATexture = AcquireTexture("a");
    ...
    GetChunksFromBox(World,
                     CameraMinPos,
                     CameraMaxPos,
                     CameraGrabEntitySafetyMargin,
                     &MinChunkX, &MinChunkY, &MinChunkZ,
                     &MaxChunkX, &MaxChunkY, &MaxChunkZ);
    
    
    #if 1
    u32 VerticesCount = 6 * World->NumTilesX *
        World->NumTilesY;
    
    SetupBatchRenderer(RenderContext, TransientArena, 10);
    RenderBegin(RenderContext, VerticesCount + 500, RENDER_ORDER_NO_ORDER);
    
    sprintf_s(Buffer, "%f, %f", Player->Velocity.X, Player->Velocity.Y);
    RenderText(RenderContext, 500.f, 500.f, DefaultFont,
               *TextureProgram, Buffer, RGBA8_WHITE, 1.f, 1.f,
               {0, 0, 30000, 30000}, 0.f);
    
    gl_texture Texture = TileMap->Texture;    
    BeginBatch(RenderContext, Texture.ID, 0.f, *TextureProgram);
    
    // Drawing Tile Map
#if 1
    u32 TextureTileNumX = Texture.Width / 32;
    u32 TextureTileNumY = Texture.Height / 32;
        ...
        TileYIndex++)
    {
            ...
            TileXIndex++)
        {
            ...
            v4 TileUvs = GetTextureUvsFromIndex(Texture.Width,
                                            Texture.Height,
                                            TextureTileNumX,
                                            TextureTileNumY,
                                            ThisTile->Index);
...
#if 1
    SetupBatchRenderer(RenderContext, TransientArena, World->EntityCount * 3);
    //TODO(zoubir): optimise this World->EntityCount
    // maybe take another value
    // 
    RenderBegin(RenderContext, World->EntityCount, RENDER_ORDER_BACK_TO_FRONT);
    for(u32 ChunkY = MinChunkY;
        ChunkY <= MaxChunkY;
        ChunkY++)
    {
            ...
            ChunkX++)
        {
                ...
                ChunkZ++)
            {
                ...
                Assert(Chunk);
                for(world_entity_chunk *EntityChunk =
                        ...
                        switch(ThisEntity->Type)
                        {
                            case EntityType_Player:
                            {
                                ...
                                ThisEntity->Uvs = DoAnimation(&ThisEntity->AnimationState, 
                                                     ThisEntity->Texture->Width, ThisEntity->Texture->Height,
    ...
    if (AppState->TileEditing)
    {
            ...
            IndexY++)
        {
                ...
                IndexX++)
            {
                ...
                float X = MouseXF + TileOffsetX;
                float Y = MouseYF + TileOffsetY;
                
                X = (float)Input->MouseX;
                Y = (float)Input->MouseY;
...
#if 0
                // Camera Offset Difference
                float DiffX = (float)((CameraOffset.X / TileWidthF) -
                                      (u32)(CameraOffset.X / TileWidthF)) *
                    TileWidthF;
                float DiffY = (float)((CameraOffset.Y / TileHeightF) -
                                      (u32)(CameraOffset.Y / TileHeightF)) *
                    TileHeightF;
...
//        Y += DiffY;
#endif   
#if 0
                u32 TileX = (u32)(X / TileWidthF) * World->TileWidth;
                u32 TileY = (u32)(Y / TileHeightF) * World->TileHeight;
                X = (float)TileX;
                Y = (float)TileY;        
#endif                
                float Width = TileWidthF;
                float Height = TileHeightF;
        
                BeginBatch(RenderContext, TileMapTexture->ID, 0.f, *TextureProgram);        
                RenderQuadTexture(RenderContext, X, Y, Width, Height,
                                  Uvs, RGBA8_WHITE, 0.f);
                EndBatch(RenderContext);
        ...
        float TilesRectangleHeight =
            (float)(TilePickerWidget->SelectedTileCountY *
                    World->TileHeight);
        DrawRectangle(RenderContext, MouseXF, MouseYF,
                      TilesRectangleWidth,
