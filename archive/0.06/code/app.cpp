          ...
          world *World, memory_arena *Arena, v3 Position)
{
    ...
    Player->TextureNumTilesX = 16;
    Player->TextureNumTilesY = 16;
          ...
          world *World, memory_arena *Arena, v3 Position)
{
    ...
    Entity->TextureNumTilesX = 16;
    Entity->TextureNumTilesY = 16;
           ...
           world_entity *Entity, v3 CameraOffset)
{
    ...
    asset_info *TextureInfo = GetAssetInfo(Assets, Entity->Texture);
    //TODO(zoubir): figure out if we need this check
    // inside assets
    if (Entity->Texture.Type)
    {
        Texture = GetTexture(Assets, Entity->Texture);
    }
    
    gl_texture *ShadowTexture = 0;
    asset_info *ShadowTextureInfo =
        GetAssetInfo(Assets, Entity->ShadowTexture);
    if (Entity->ShadowTexture.Type)
    {
        ShadowTexture = GetTexture(Assets, Entity->ShadowTexture);
    }
...
extern "C" APP_UPDATE_AND_RENDER(AppUpdateAndRender)
{
    ...
    if (!AppState->IsInitialized)
    {
        ...
        InitializeAssets(Assets, MemoryArena);
        InitializeAudio(AudioState);

        BeginAssetType(Assets, AssetType_Dragon, 1);
        AddTextureAsset(Assets, "a.zi", {}, TEXTURE_SOFT_FILTER);
        EndAssetType(Assets);
        BeginAssetType(Assets, AssetType_Zoubir, 1);
        AddTextureAsset(Assets, "move-shizuka.zi", {0.5f, 0.875f}, TEXTURE_SOFT_FILTER);
        EndAssetType(Assets);
        BeginAssetType(Assets, AssetType_Shadow, 1);
        AddTextureAsset(Assets, "player_shadow.zi", {0.5f, 0.5f}, TEXTURE_SOFT_FILTER);
        EndAssetType(Assets);
        BeginAssetType(Assets, AssetType_TileMap, 1);
        AddTextureAsset(Assets, "forest_pallet_01.zi", {}, TEXTURE_NO_FILTER);
        EndAssetType(Assets);
        BeginAssetType(Assets, AssetType_Tree, 1);
        AddTextureAsset(Assets, "tree_summer_01.zi", {0.5f, 0.931f}, TEXTURE_SOFT_FILTER);
        EndAssetType(Assets);
        BeginAssetType(Assets, AssetType_Familiar, 1);
        AddTextureAsset(Assets, "familiar.zi", {0.5f, 0.5f}, TEXTURE_SOFT_FILTER);
        EndAssetType(Assets);
        BeginAssetType(Assets, AssetType_FireBall, 1);
        AddTextureAsset(Assets, "Fireball.zi", {0.5f, 0.5f}, TEXTURE_SOFT_FILTER);
        EndAssetType(Assets);
        BeginAssetType(Assets, AssetType_Sword, 1);
        AddTextureAsset(Assets, "sword1.zi", {0.5f, 0.7f}, TEXTURE_SOFT_FILTER);
        EndAssetType(Assets);
        ...
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_Dragon});
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_Zoubir});
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_Shadow});
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_TileMap});
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_Tree});
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_Familiar});
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_FireBall});
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_Sword});

        
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_ZoubirAudio});
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_Dash});
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_FireCast});
        LoadAsset(Assets, AppState, Memory, MemoryArena, {AssetType_BattleTheme});
        ...
        AppState->PlayerCollision = 
            MakeSimpleGroundedCollisionVolume(ConstantsArena, {10, 4, 19.f});
        AppState->FamiliarCollision = 
...
//                Tiles[CurrentTile].Index = 4;
    ...
    gl_texture *DragonTexture = GetTexture(Assets, {AssetType_Dragon});
    gl_texture *ZoubirTexture = GetTexture(Assets, {AssetType_Zoubir});
        ...
        switch(ThisEntity->Type)
        {
            ...
            case EntityType_Familiar:
            {
                ...
                ThisEntity->Uvs = DoAnimation(&ThisEntity->AnimationState, 
                                              ...
                                              Input->DeltaTime, EntityAnimationSpeed,
                                              ThisEntity->AnimationSet,
                                              EntityAnimationType,
                                              EntityAnimationDirection);
                }
            ...
            case EntityType_FireBall:
            {
                ...
                //TODO(zoubir): we are calling GetTexture Twice One here
                // and one inside the draw Entity
                gl_texture *Texture = GetTexture(Assets, ThisEntity->Texture);
                if (Texture)
                {
                ThisEntity->Uvs = DoAnimation(&ThisEntity->AnimationState, 
            ...
            case EntityType_Monster:
            {
                ...
                animation_type EntityAnimationType = AnimationType_Stand;
                animation_direction EntityAnimationDirection =
                    AnimationDirection_Down;
...
//    FrameTemporaryMemory = BeginTemporaryMemory(TransientArena);
    ...
    if (AppState->TileEditing)
    {
        ...
        DoTilePickerWidget(&AppState->TilePickerWidget, AppState,
                           ...
                           GetTexture(Assets, {AssetType_TileMap}), 32, 32,
                           8, 200);
