...
#include "intrin.h"
internal font *
CreateFont(thread_context *Thread, memory_arena *Arena,
              ...
              char *FilePath)
{
    ...
    debug_read_file_result ReadResult =
        ReadEntireFile(Thread, FilePath);
    ...
    FreeFileMemory(Thread, ReadResult.Memory);
...
internal gl_texture
LoadTexture(thread_context *Thread, app_memory *Memory, char *FileName, u32 Flags)
{
    gl_texture Result = {};
    debug_read_file_result FileResult =
        Memory->DEBUGPlatformReadEntireFile(Thread, FileName);
    
    if (FileResult.Size < sizeof(zi_header)) return Result;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, ImageFormat, Header->Width, Header->Height, 0, ImageFormat, GL_UNSIGNED_BYTE, Data);
                 ...
                 render_program *Program)
{
        ...
        RenderContext->LastUsedProgramID != Program->ID)
    {
        ...
        i32 MatrixLocation = glGetUniformLocation(Program->ID, "P");
        glUniformMatrix4fv(MatrixLocation, 1, GL_FALSE, Program->ProjectionMatrix->Data);
...
inline void
RenderProgramUnuse(render_program *Program)
{
    glUseProgram(0);                
    for(u32 AttribIndex = 0;
        AttribIndex < Program->NumAttrib;
...
RenderFlush(render_context *RenderContext)
{
    ...
    switch(RenderContext->RendererType)
    {
        ...
        {       
            ...
            glBufferData(GL_ARRAY_BUFFER, VerticesMemoryBlockSize, 0, GL_DYNAMIC_DRAW);
            glBufferSubData(GL_ARRAY_BUFFER, 0, VerticesMemoryBlockSize, VerticiesToRender);
