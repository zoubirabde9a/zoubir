...
//                                MinCorner.Z += Entity->CollisionHalfDims.Z;
                                        v3 MaxCorner = MinkowskiDiameter;
                                        //TODO(zoubir): look at this mess
//                              MaxCorner.Z += ThisEntity->CollisionHalfDims.Z;
//                                MaxCorner.Z -= Entity->CollisionHalfDims.Z;
                                    
                                        v3 Rel = (EntityOldPosition + Volume->Offset) -
                                            (ThisEntity->Position + TestVolume->Offset);
                
                                        if (TestWall(MinCorner.X, Rel.X, Rel.Y, Rel.Z,
                                                     EntityDelta.X, EntityDelta.Y, EntityDelta.Z,
                                        ...
                                        if (TestWall(MaxCorner.X, Rel.X, Rel.Y, Rel.Z,
                                                     EntityDelta.X, EntityDelta.Y, EntityDelta.Z,
                                                     &tMin, MinCorner.Y, MaxCorner.Y,
                                                     MinCorner.Z, MaxCorner.Z))
                                        {
                                        ...
                                        if (TestWall(MinCorner.Y, Rel.Y, Rel.X, Rel.Z,
                                                     EntityDelta.Y, EntityDelta.X, EntityDelta.Z,
                                        ...
                                        if (TestWall(MaxCorner.Y, Rel.Y, Rel.X, Rel.Z,
                                                     EntityDelta.Y, EntityDelta.X, EntityDelta.Z,
                                                     &tMin, MinCorner.X, MaxCorner.X,
                                                     MinCorner.Z, MaxCorner.Z))
                                        {
                                        ...
                                        if (TestWall(MinCorner.Z, Rel.Z, Rel.Y, Rel.X,
                                                     EntityDelta.Z, EntityDelta.Y, EntityDelta.X,
                                                     &tMin, MinCorner.Y, MaxCorner.Y,
                                        ...
                                        if (TestWall(MaxCorner.Z, Rel.Z, Rel.Y, Rel.X,
                                                     EntityDelta.Z, EntityDelta.Y, EntityDelta.X,
                                                     &tMin, MinCorner.Y, MaxCorner.Y,
                                                     MinCorner.X, MaxCorner.X))
                                        {
                                        ...
                                        if (TestWall(MinCorner.Z, Rel.Z, Rel.X, Rel.Y,
                                                     EntityDelta.Z, EntityDelta.X, EntityDelta.Y,
                                                     &tMin, MinCorner.X, MaxCorner.X,
                                        ...
                                        if (TestWall(MaxCorner.Z, Rel.Z, Rel.X, Rel.Y,
                                                     EntityDelta.Z, EntityDelta.X, EntityDelta.Y,
                                                     &tMin, MinCorner.X, MaxCorner.X,
                                                     MinCorner.Y, MaxCorner.Y))
                                        {
...
extern "C" APP_UPDATE_AND_RENDER(AppUpdateAndRender)
{
    ...
    {        
        ...
        AppState->TreeCollision =
            MakeGroundedTreeCollisionVolume(MemoryArena);
        AppState->WallCollision = 
            MakeSimpleGroundedCollisionVolume(MemoryArena, {TileWidth*.5f, TileHeight*.5f, TileDepth *.5f});
        AppState->PlayerCollision = 
            MakeSimpleGroundedCollisionVolume(MemoryArena, {10, 4, 18.f});
        AppState->FamiliarCollision = 
            MakeSimpleGroundedCollisionVolume(MemoryArena, {11, 6, 0.f});
        AppState->FireBallCollision = 
            MakeSimpleGroundedCollisionVolume(MemoryArena, {9, 9, 0.f});
        ...
        World->MaxEntityVelocity = {4.f, 4.f, 4.f};
            ...
            MonsterIndex++)
        {
            AddMonster(AppState, World,
                       ...
                       {((MonsterIndex % 3) * 32.f + MonsterIndex) * 32.f,
                               200 +
                               (float)((MonsterIndex % 2) * 5 + MonsterIndex) * 10,
                               250.f});
        }
        AddMonster(AppState, World,
                   MemoryArena,
...
//  gl_texture ATexture = AcquireTexture("a");
    ...
    RenderBegin(RenderContext, VerticesCount, RENDER_ORDER_NO_ORDER);
...
#if 1
    SetupBatchRenderer(RenderContext, TransientArena, World->EntityCount * 2);
